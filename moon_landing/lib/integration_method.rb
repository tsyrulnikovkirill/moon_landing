...
def runge_kutta(arr_parameters, t, t_end, d_pitch_dt, pitch_2, excel)
    ...
    while t < t_end
        ...
        k_v_x_1 = dt * dv_x_dt(pitch_angle, x_old, y_old, t)
        k_v_y_1 = dt * dv_y_dt(pitch_angle, x_old, y_old, t)
        k_x_1 = dt * dx_dt(v_x_old)
        k_y_1 = dt * dy_dt(v_y_old)
        # k_m_1 = dt * dm_dt(t)

        k_v_x_2 = dt * dv_x_dt(pitch_angle, x_old + k_x_1 / 2, y_old + k_y_1 / 2, t + dt / 2)
        k_v_y_2 = dt * dv_y_dt(pitch_angle, x_old + k_x_1 / 2, y_old + k_y_1 / 2, t + dt / 2)
        k_x_2 = dt * dx_dt(v_x_old + k_v_x_1 / 2)
        k_y_2 = dt * dy_dt(v_y_old + k_v_y_1 / 2)
        # k_m_2 = dt * dm_dt(t + dt / 2)

        k_v_x_3 = dt * dv_x_dt(pitch_angle, x_old + k_x_2 / 2, y_old + k_y_2 / 2, t + dt / 2)
        k_v_y_3 = dt * dv_y_dt(pitch_angle, x_old + k_x_2 / 2, y_old + k_y_2 / 2, t + dt / 2)
        k_x_3 = dt * dx_dt(v_x_old + k_v_x_2 / 2)
        k_y_3 = dt * dy_dt(v_y_old + k_v_y_2 / 2)
        # k_m_3 = dt * dm_dt(t + dt / 2)

        k_v_x_4 = dt * dv_x_dt(pitch_angle, x_old + k_x_3, y_old + k_y_3, t + dt)
        k_v_y_4 = dt * dv_y_dt(pitch_angle, x_old + k_x_3, y_old + k_y_3, t + dt)
